...
telegraph = Telegraph()
telegraph.create_account(short_name='PremiumScrapp')

# Ruta de la imagen .jpg
imagen_path = "photo_2024-11-05_20-33-36.jpg" 
...
def generate_extra_info(cc):
    ...
    while len(extras) < 2:
        ...
        masked_cc = f"{base_cc}{random_digits}xxxx"
        if masked_cc not in unique_numbers:
            ...
            extra = f"│ {len(extras) + 1} {masked_cc}|{exp_month}|{exp_year}|rnd"
            extras.append(extra)
...
𝗡𝗲𝘅𝗼 𝗦𝗰𝗿𝗮𝗽𝗽𝗲𝗿 ㄌ
━━━━━━━━━⁐━━━━━━━━━
- 𝗧𝗮𝗿𝗷𝗲𝘁𝗮: <code>{cc}|{mes}|{ano}|{cvv}</code>
- 𝗕𝗮𝗻𝗰𝗼: {bank}
- 𝗧𝗶𝗽𝗼: {brand}
- 𝗣𝗮í𝘀: {country} {flag}
- 𝗡𝗶𝘃𝗲𝗹: {level}
...
async def main():
    while True:
        try:
            client = TelegramClient('PremiumScrapp', id_api, api_hash)
